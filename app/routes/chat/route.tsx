    ...
    <main className="h-screen w-screen flex flex-col items-center overflow-y-auto">
      ...
      <div className="w-9/12 flex flex-col gap-4 overflow-y-auto">
        {messages.map((msg) => (
          <div
            key={msg.id}
            className="flex flex-row gap-4 items-center bg-foreground "
          >
            <svg
              ...
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              className="size-12 bg-accent fill-background rounded"
            >
              <path
                strokeLinecap="round"
      ...
      <Form
        method="POST"
        className="fixed bottom-4 p-4 flex flex-row items-center bg-foreground rounded-lg w-9/12"
      >
        <input
          name="message"
